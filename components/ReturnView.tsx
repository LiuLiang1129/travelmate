...
import { ReturnInfo, User } from '../types';
...
const ReturnView: React.FC<ReturnViewProps> = ({ returnInfo, canManage, onEdit }) => {

  if (!returnInfo.segments || returnInfo.segments.length === 0) {
    return (
      <div className="w-full max-w-4xl mx-auto p-4 md:p-6 lg:p-8">
         <div className="flex justify-between items-center mb-6">
            <h2 className="text-3xl font-bold text-gray-800">返程資訊</h2>
             {canManage && (
                <button 
                    onClick={onEdit}
                    className="flex items-center bg-indigo-600 text-white font-semibold py-2 px-4 rounded-lg shadow-md hover:bg-indigo-700 transition"
                >
        ...
        <div className="text-center py-16 px-6 bg-white rounded-lg shadow-sm border border-dashed border-gray-300">
             <h3 className="text-xl font-medium text-gray-700">尚未新增返程資訊。</h3>
  ...
  const firstSegment = returnInfo.segments[0];
  const lastSegment = returnInfo.segments[returnInfo.segments.length - 1];

  const returnDate = new Date(firstSegment.departureDateTime);
  const returnDateString = returnDate.toLocaleDateString('zh-TW', { year: 'numeric', month: 'long', day: 'numeric' });
  const returnTimeString = returnDate.toLocaleTimeString('zh-TW', { hour: '2-digit', minute: '2-digit', hour12: false });
  const returnWeekdayString = returnDate.toLocaleDateString('zh-TW', { weekday: 'long' });
  ...
  const completedChecklistItems = returnInfo.checklist.filter(item => item.isChecked).length;
  const totalChecklistItems = returnInfo.checklist.length;
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 mb-8 border-l-4 border-indigo-500">
        <div className="flex flex-col md:flex-row justify-between items-start">
            <div className="flex-grow">
                <p className="text-sm font-semibold text-gray-500">{returnInfo.segments.map(s => s.transportMode).join(' → ')}</p>
                <div className="flex items-center space-x-4 my-2">
                    <p className="text-2xl font-bold text-gray-800">{firstSegment.origin}</p>
                    <PaperAirplaneIcon className="w-8 h-8 text-indigo-500 transform -rotate-135" />
            ...
            <div className="text-left md:text-right mt-4 md:mt-0 flex-shrink-0 md:pl-6 space-y-2">
                <div>
                    <p className="text-sm font-semibold text-indigo-600">返程</p>
                    <p className="text-xl font-bold text-gray-800">{returnTimeString}</p>
                    <p className="text-gray-600">{returnDateString} ({returnWeekdayString})</p>
                </div>
                {lastSegment.arrivalDateTime && arrivalTimeString && (
                <div>
                    <p className="text-sm font-semibold text-green-600">返抵 (當地時間)</p>
        ...
        <div className="space-y-8">
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
                ...
                <div className="space-y-6">
                    {returnInfo.segments.map((segment, index) => (
                        <div key={segment.id}>
                            ...
                            {index < returnInfo.segments.length - 1 && (
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
                ...
                <ul className="space-y-2 list-disc list-inside text-gray-700">
                    {returnInfo.reminders.map((reminder, index) => (
