...
import { DepartureInfo, ChecklistItem, User, TransportSegment } from '../types';
import { PlusIcon, TrashIcon, ClockIcon } from './icons';

interface DepartureInfoModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (info: DepartureInfo) => void;
...
const DepartureInfoModal: React.FC<DepartureInfoModalProps> = ({ isOpen, onClose, onSave, departureInfo, currentUser, canManage }) => {
  const [formData, setFormData] = useState<DepartureInfo>(departureInfo);

  useEffect(() => {
    setFormData(departureInfo);
  }, [departureInfo]);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] flex flex-col">
        <div className="p-6 border-b">
          <h2 className="text-2xl font-bold text-gray-800">編輯出發資訊</h2>
        </div>
              ...
              {formData.segments.map((segment, index) => (
                <div key={index} className="p-4 border rounded-lg relative bg-gray-50">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700">出發日期與時間</label>
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700">抵達當地日期與時間</label>
...
export default DepartureInfoModal;
