...
import { ReturnInfo, ChecklistItem, User, TransportSegment } from '../types';
import { PlusIcon, TrashIcon, ClockIcon } from './icons';

interface ReturnInfoModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (info: ReturnInfo) => void;
...
const ReturnInfoModal: React.FC<ReturnInfoModalProps> = ({ isOpen, onClose, onSave, returnInfo, currentUser, canManage }) => {
  const [formData, setFormData] = useState<ReturnInfo>(returnInfo);

  useEffect(() => {
    setFormData(returnInfo);
  }, [returnInfo]);
  ...
  const handleSegmentDateTimeChange = (index: number, field: 'departureDateTime' | 'arrivalDateTime', value: string) => {
    const newSegments = [...formData.segments];
    // By appending 'Z', we tell the Date constructor to parse the string from the datetime-local input as UTC.
    // This prevents the user's local timezone from causing an incorrect time shift when converting to an ISO string.
    const dateValue = value ? new Date(value + 'Z').toISOString() : undefined;
    
    // Annoying type error without casting
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] flex flex-col">
        <div className="p-6 border-b">
          <h2 className="text-2xl font-bold text-gray-800">編輯返程資訊</h2>
        </div>
              ...
              {formData.segments.map((segment, index) => (
                <div key={index} className="p-4 border rounded-lg relative bg-gray-50">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700">返程日期與時間</label>
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700">返抵當地日期與時間</label>
                    ...
                    <div className="md:col-span-2">
                        <div className="flex items-center text-sm text-gray-600 bg-gray-100 p-2 rounded-md border">
                            ...
                            <span className="font-medium text-gray-500 mr-2">交通時間:</span>
                            <span className="font-semibold text-gray-800">{calculateDuration(segment.departureDateTime, segment.arrivalDateTime)}</span>
                        </div>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">返程地 (出發)</label>
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700">目的地 (返抵)</label>
