...
import { ChatBubbleIcon, PlusIcon, TrashIcon } from './icons';
...
const DiscussionForumView: React.FC<DiscussionForumViewProps> = ({ threads, currentUser, onAddReply, onOpenCreateThreadModal, onDeleteThread }) => {
        ...
        {sortedThreads.map(thread => (
          ...
          >
            <div className="flex justify-between items-start">
              ...
              <div className="flex items-center text-sm text-gray-500 flex-shrink-0 ml-4 gap-3">
                <div className="flex items-center">
                  <ChatBubbleIcon className="w-4 h-4 mr-1.5" />
                  <span>{thread.replies.length}</span>
                </div>
                {(currentUser.role === 'admin' || currentUser.role === 'tour_leader' || thread.author.id === currentUser.id) && (
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      onDeleteThread(thread.id);
                    }}
                    className="p-1 text-gray-400 hover:text-red-600 rounded-full hover:bg-gray-100 transition"
                    aria-label="刪除討論"
                  >
